...
static int test_create_publickey() {
  ...
  if (ret != 0) {
	...
	if(atkey.atkeytype != ATCLIENT_ATKEY_TYPE_PUBLICKEY) {
		atclient_atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "atkey type is not ATCLIENT_ATKEY_TYPE_PUBLICKEY, it is %d\n", atkey.atkeytype);
		ret = 1;
		goto exit;
	}
...
static int test_create_selfkey() {
	...
	if(atkey.atkeytype != ATCLIENT_ATKEY_TYPE_SELFKEY) {
		atclient_atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "atkey type is not ATCLIENT_ATKEY_TYPE_SELFKEY, it is %d\n", atkey.atkeytype);
		ret = 1;
		goto exit;
	}
...
static int test_create_sharedkey() {
	...
	ret = atclient_atkey_create_sharedkey(&atkey, "name", strlen("name"), "@jeremy", strlen("@jeremy"), "@chess69lovely", strlen("@chess69lovely"), "wavi", strlen("wavi"));
	if(ret != 0) {
	...
	if(atkey.atkeytype != ATCLIENT_ATKEY_TYPE_SHAREDKEY) {
		atclient_atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "atkey type is not ATCLIENT_ATKEY_TYPE_SHAREDKEY, it is %d\n", atkey.atkeytype);
		ret = 1;
		goto exit;
	}
	...
	if(strcmp(atkey.sharedwith.str, "@chess69lovely") != 0) {
		atclient_atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "atkey.sharedwith.str is not @chess69lovely, it is \"%s\"\n", atkey.sharedwith.str);
		ret = 1;
		goto exit;
	}
