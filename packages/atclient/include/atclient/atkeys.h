...
/**
 * @brief populates the struct by decrypting the encrypted RSA keys passed. It is assumed that the passed strings are encrypted RSA keys that were in base64 format.
 *
 ...
 * @param aesencryptpublickeystr  the encrypted RSA public key (encrypted with AES-256 selfencryptionkey) in base64 format
 * @param aesencryptprivatekeystr the encrypted RSA private key (encrypted with AES-256 selfencryptionkey) in base64 format
