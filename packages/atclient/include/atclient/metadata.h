...
typedef struct atclient_atkey_metadata
{
    ...
    atclient_atstr sharedkeyenc;  // stores the shared key that the data is encrypted with. this is only set if sharedWith is set. the contents will be encrypted using the public key of the sharedWith atSign
    atclient_atstr pubkeycs;      // public key checksum. Stores the checksum of the encryption public key used to encrypt the [sharedkeyenc]. We use this to verify that the encryption keypair used to encrypt and decrypt the value are teh same
    atclient_atstr ivnonce;       // Initialization vector or nonce used when the data was encrypted with the shared symmetric encryption key
    atclient_atstr enckeyname;    // the name of the key used to encrypt the value. If not provided, use sharedKeyEnc in the metadata. If sharedKeyEnc is not provided, use the default shared key. If enckeyname is provided, just the key name must be provided example without the sharedWith suffix and sharedBy prefix, nor visibility prefix. Example '@bob:shared_key.wavi@alice', must be only be 'shared_key.wavi'
    atclient_atstr encalgo;       // name of the algorithm used to encrypt the value. For data, the default algorithm is 'AES/SIC/PKCS7Padding', for cryptographic keys, the default algorithm is 'RSA'
